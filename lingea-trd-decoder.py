...
#                 added support for French-Czech and Spanish-Czech dictionaries
#                 automatic encoding selection
#                 all unrecognized characters are printed now as #something#
#                    (note: some of them are bugs in Lingea dictionaries)
#                 typo
...
# Generalization of data block rules, sampleFlag 0x04
# sound out fix, data phrase prefix with comment (0x04)
...
try:
    opts, args = getopt.getopt(
        sys.argv[1:],
        "hdo:ral:e:",
        ["help", "debug", "out-style=", "debug-header", "debug-all", "debug-limit="],
    )
...
if OUTSTYLE == 2:
    tag = {
        ...
        'pc':('    <span style="color: darkgreen; font-style: italic">', '</span> '),
        # Data phrase comment; this comment is not printed by Lingea, but it seems useful
...
def decode_tag_postprocessing(input_result):
...
def outInt(comment=""):
    ...
    if DEBUG:
        print("%03d %s %s | %03d" % (pos, toBin(bs[pos]), comment, pos))
    if (comment.find('%') != -1):
...
def outStr(comment=""):
    ...
    if (comment.find('%') != -1):
        comment = comment % s
    if DEBUG:
        print("%03d %s %s | %s" % (pos, toBin(bs[pos]), comment, s))
    pos += triple + 1
    return s.replace('`','')  # Remove '`' character from words


def decode(stream):  # noqa
    ...
    if mainFlag & 0x01:
        ...
        if headerFlag & 0x04:
            ...
            if s < 32:
                ...
                raise Exception("Header wordclass out of range in: %s" % result)
        if headerFlag & 0x08:
    ...
    ok = True
    if (
            len(stream) != 13752 and
            len(stream) != 21988 and
            len(stream) != 16204 and
            len(stream) != 12656 and
            len(stream) != 30696
    ):  # hack to workaround bug in some dicts (lg_czen-eco, lg_encz-ind, lg_czgr-eco, lg_grsk-2, lg_grcz-2.trd)
...
copyright = unpack("<64s",f.read(64))[0]  # noqa
...
if smallIndex:  # TODO: smallIndex might not correspond with encoding
    alpha = [
        '\x00', 'a','b','c','d','e','f','g','h','i',
        'j','k','l','m','n','o','p','q','r','s',
        't','u','v','w','x','y','z','á','ä','č',
        'ď','é', 'ě', 'í', '#AL34#', '#AL35#', 'ň', 'ó', 'ö', '#AL39#',
        'ř', 'š', 'ť', 'ú', 'ů', 'ü', 'ý', 'ž', 'ß', ' ',
        '.', ',', '-', '\'', '(', ')', '`', '"', '#AL58#', '#AL59#',
        '#UPCASE#', 'à', '#SPECIAL#', "#AL1234213",
    ]  # 4 bytes after unicode
    ...
    alpha = [
        ...
        '#STROKE#', '#SHARP#', 'β', '#AL53#', '#AL54#', '#AL55#', '#AL56#', '#AL57#', 's', '#SYMBOL#',
        # symbol 58 is used in Spanish word pillo as s (seimpre)
        '#PRON#', '#UPCASE#', '#SPECIAL#', '#UNICODE#',
    ]  # 4 bytes after unicode
...
upcase_pron = [
    '#upr0#', '#upr1#','#upr2#','#upr3#','#upr4#','#upr5#','#upr6#','#upr7#','#upr8#','#upr9#',
...
wordclass = (
    'subs:','n:','adj:','pron:','num:','v:','adv:','prep:','conj:','part:',
...
for b in bases:
    ...
    for o in offsets:
        if len(index) < indexOffsetCount:
            # print "Index %s: %s + %s + %s * 4 = %s" % (
            # len(index), bodyPos, b, o, toBin(bodyPos + b + o * 4))
...
if DEBUG:
    ...
    for i in range(1, entryCount):
        ...
        if s.endswith('\n'):
            ...
            print("Please run this script in DEBUG mode and repair ")
            print("DATA BLOCK(S) section in function decode()")
            print("If you succeed with whole dictionary send report ")
            print("(name of the dictionary and source code of script) to slovniky@googlegroups.com")
            break
